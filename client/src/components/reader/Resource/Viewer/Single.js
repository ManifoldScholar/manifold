...
class ResourceViewerSingle extends PureComponent {
  ...
  handleDestroy(eventIgnored) {
    const heading = "Are you sure you want to remove this resource?";
    const message =
      "Pressing yes will remove the resource from this spot in the text. " +
  ...
  doDestroy() {
    ...
    const options = {
      removes: { type: "annotations", id: this.props.annotationId }
    };
  ...
  renderThumbnail() {
    ...
    const hasImage = !!get(
      resource,
      `attributes.attachmentStyles['${variant}']`
    );
    ...
    return (
      <div
        ...
        onMouseOver={() => {
          this.setActiveAnnotation(this.props.annotationId);
        }}
        onMouseLeave={() => {
...
export default connect(ResourceViewerSingle.mapStateToProps)(
