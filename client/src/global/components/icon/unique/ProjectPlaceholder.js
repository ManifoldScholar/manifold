...
function classes(className, type, color) {
  const colorModifier = !color || color === "sentary" ? "sentary" : color;

  return classNames(
    blockClass,
    className,
    `${blockClass}--${type}`,
    `${blockClass}--${colorModifier}`
  );
}
    ...
    <>
      ...
      >
        ...
        <g fill="none" fillRule="evenodd">
          ...
          {!icon && (
            <g
              className={`${blockClass}__illustration`}
          ...
          {!borderless && (
            <g className={`${blockClass}__frame`} stroke="#828282">
...
function Large({ type, className, color, ariaLabel, icon }) {
  return (
    <>
      <svg
        xmlns="http://www.w3.org/2000/svg"
...
ProjectPlaceholder.displayName = "SVG.ProjectPlaceholder";
