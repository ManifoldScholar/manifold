...
const devLogFormat =
  "[WEB] :method :url :status :response-time ms - :res[content-length]";
const logStyle =
...
function start(stats) {
      ...
      const render = () => {
        try {
          ...
          if (error.code === "MODULE_NOT_FOUND") {
            ...
            ch.error(
              `Universal render fallback failed to render in server-web.js`
            );
  ...
  ch.info(
    `Proxying all other requests to the rendering service at ${universalTarget}`
  );
...
fs.readFile(statsPath, "utf8", (err, data) => {
  if (err) {
    ch.error(
      "Manifold's Node web server is starting without a client asset manifest,"
    );
    ch.error(
