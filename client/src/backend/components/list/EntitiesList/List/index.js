...
export default class ListEntities extends PureComponent {
  ...
  static errors = {
    callbackInvalid: 'List.Entities.List "callback" prop must be null or plain object.',
    missingOnPageClick:
    ...
    showCountNoPagination: 'Lists that show count must have a pagination object prop.',
    invalidButton:
  ...
  static validateCallbacks = (props, propName) => {
    ...
    const missingOnPageClick = (props.pagination && !isFunction(callbacks.onPageClick));
  ...
  static validateShowCounts = (props, propName) => {
    ...
    if (value && !props.pagination) return new Error(this.errors.showCountNoPagination);
  };

  static validateEnsureButton = (propValue, key) => {
    const value = propValue[key];
    if (!value || !value.type || value.type.displayName !== "List.Entities.List.Button") {
      ...
      <div className={wrapperClassNames}>
        ...
        <div className={contentsWrapperClassName}>
          ...
          <div className="entity-list__header">
            {this.hasButtons && (
              <ButtonSet buttons={this.buttons} />
            )}
            {this.showCount && (
