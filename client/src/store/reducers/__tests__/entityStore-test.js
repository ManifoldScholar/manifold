...
describe('store/reducers/entityStore', () => {
  ...
  it('should return the initial state', () => {
    const state = entityStoreReducer(undefined, { type: "SOME UNRELATED ACTION" });
    expect(state).toEqual(initialState);
  });

  it('should store a single model in the entity story', () => {
    const state = entityStoreReducer(
      undefined,
      {
        meta: "test-response",
  ...
  it('should not overwrite an existing entity with a partial entity', () => {
    const stateA = entityStoreReducer(
      undefined,
      {
        meta: "test-response",
        payload: apiShowResponse,
    ...
    const stateB = entityStoreReducer(
      stateA,
      {
        meta: "test-response",
        payload: apiCollectionResponse,
