...
function start(stats) {
  ...
  const devLogFormat =
    "[REND] :method :url :status :response-time ms - :res[content-length]";
  const logStyle =
  ...
  if (listenOn) {
    server.listen(listenOn, err => {
      ...
      ch.header(
        `Manifold rendering service engaged on port ${config.universalServerPort}`
      );
    });
  } else {
    ch.error(
      `No CLIENT_UNIVERSAL_SERVER_PORT environment variable has been specified`
    );
  }
  ...
  process.once("SIGUSR2", () => {
    ...
    ch.info(
      "   While it's being restarted, there will be no server-side rendering.",
      null
    );
