...
class ProjectDetailTextNew extends PureComponent {
  ...
  static fetchData(getState, dispatch, location, params) {
    if (!params.ingestionId) return;
    if (entityUtils.isLoaded(requests.beIngestionCreate, getState())) return;
  ...
  redirectToIngest() {
    browserHistory.push(this.ingestUrl());
  }
  ...
  componentWillMount() {
    if (this.props.params.step) {
      this.setState({ step: parseInt(this.props.params.step, 10) });
    }
  }
...
export default connect(
  ProjectDetailTextNew.mapStateToProps
)(withRouter(ProjectDetailTextNew));
