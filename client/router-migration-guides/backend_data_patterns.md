...
export default function NewPageRoute() {
  ...
  return (
    ...
    >
      {/* form fields — errors handled automatically */}
    </FormContainer.Form>
  );
}
```
...
Return `{ success: true }` instead of redirecting. React Router auto-revalidates the parent layout loader after the action, refreshing the entity data.

```js
// Action in a child route (e.g. $id/properties.jsx)
...
// Component — page comes from parent layout via outlet context
export default function PropertiesRoute() {
  const { page } = useOutletContext();
  const fetcher = useFetcher();
  return (
    <FormContainer.Form fetcher={fetcher} model={page} className="form-secondary">
      {/* form fields */}
    </FormContainer.Form>
  );
}
