...
RSpec.describe Ingestor::Strategy::EPUB::Strategy do
  ...
  shared_examples "output text assertions" do |source_root, unique_identifier|
    describe "the text generated by the ingestion" do
      ...
      it "has a title that matches the dublin core language metadata" do
        expect(@text.metadata["language"]).to eq "en"
      end

      it "has a unique_identifier that matches the one specified in the EPUB source" do
        expect(@text.metadata["unique_identifier"]).to eq unique_identifier
      end
  ...
  context "when ingesting from url", :slow, :integration do
    ...
    describe "the text generated by the ingestion" do
      ...
      it "has a title that matches the dublin core language metadata" do
        expect(@text.metadata["language"]).to eq "en-GB"
      end

      it "has a title that matches the dublin core title metadata" do
        expect(@text.title).to eq "Master Flea"
      end

      it "has a title that matches the dublin core rights metadata" do
        expect(@text.metadata["rights"]).to start_with("The source text and artwork in this ebook edition") &&
          end_with("Uncopyright file included at the end of this ebook.")
      end

      it "has a title that matches the dublin core description metadata" do
        description = "A gentleman bachelor gains an epic advantage of tiny proportions in this allegorical, Romantic, fairy-tale novel."
        expect(@text.description).to eq description
      end

      it "has the correct number of text sections" do
        expect(@text.text_sections.length).to be 12
      end
      ...
      it "has the correct number of spine entries" do
        expect(@text.spine.length).to eq 12
      end

      it "has a text_section for each spine entry" do
        expect(@text.text_sections.length).to eq 12
      end

      it "has an ingestion_source for each manifest item" do
        expect(@text.ingestion_sources.length).to eq 18
      end

      it "sets the creator correctly" do
        author = @text.creators.first
        expect(author.first_name).to eq "E. T. A."
        expect(author.last_name).to eq "Hoffmann"
      end
      ...
      describe "the generated ingestion sources" do
        ...
        it "has a source_path property equal to the relative path within the package" do
          section = @text.text_sections.first
          expect(section.source_path).to eq "epub/text/titlepage.xhtml"
        end
      ...
      describe "its table of contents" do
        ...
        it "has the correct number of entries" do
          toc = @text.toc
          expect(toc.length).to eq 12
        end

        it "has the correct labels" do
          target_toc = ["Title Page", "Imprint", "I: First Adventure", "II: Second Adventure", "III: Third Adventure", "IV: Fourth Adventure", "V: Fifth Adventure", "VI: Sixth Adventure", "VII: Seventh Adventure", "Endnotes", "Colophon", "Uncopyright"]
          toc = @text.toc
          expect(toc.map { |i| i[:label] }).to eq target_toc
        end
