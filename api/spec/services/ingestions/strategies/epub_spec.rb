...
RSpec.describe Ingestions::Strategies::Epub do
  shared_examples "outcome assertions" do
    ...
    describe "its relationships" do
      ...
      it "has the correct maker attributes" do
        expected = [
          { "name" => "Andrew Culp", "sort_name" => "" }
        ]
        expect(manifest[:relationships][:makers]).to eq expected
      end
  ...
  context "when url" do
    before(:all) do
      @path = "https://standardebooks.org/ebooks/e-t-a-hoffmann/master-flea/george-soane/dist/e-t-a-hoffmann_master-flea.epub3"
