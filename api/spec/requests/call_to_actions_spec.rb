...
RSpec.describe "Call To Actions API", type: :request do
  include_context("authenticated request")
  include_context("param helpers")

  let(:cta) { FactoryBot.create(:call_to_action) }
  let(:path) { api_v1_call_to_action_path(cta) }
      ...
      describe "the response" do
        ...
        context "body" do
          ...
          it "contains the updated relationships" do
            toc_cta = FactoryBot.create(:table_of_contents_cta)
            new_text = FactoryBot.create(:text, project: toc_cta.project)
            path = api_v1_call_to_action_path(toc_cta)
