module ResourceAttachmentValidation
  ...
  def validate_video_fields
    ...
    errors.add(:attachment, "video is required") unless (external_url.present? && external_type.present?) || attachment.present?
    errors.add(:attachment, "is invalid video file") unless (external_url.present? && external_type.present?) || attachment_is_video?
    errors.empty?
  end
