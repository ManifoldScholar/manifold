SELECT
    ...
    COUNT(DISTINCT a.id) FILTER (WHERE a.creator_id = rgm.user_id AND a.format = 'annotation' AND NOT a.orphaned AND a.text_section_id IS NOT NULL) AS annotations_count,
    ...
    COUNT(DISTINCT a.id) FILTER (WHERE a.creator_id = rgm.user_id AND a.format = 'highlight' AND NOT a.orphaned AND a.text_section_id IS NOT NULL) AS highlights_count,
