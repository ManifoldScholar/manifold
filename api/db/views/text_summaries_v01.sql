SELECT
  ...
  LEFT JOIN LATERAL (
    SELECT
      (string_agg(m.cached_full_name, ', ') FILTER (WHERE role = 'creator')) AS creator_names,
      (string_agg(m.cached_full_name, ', ') FILTER (WHERE role = 'collaborator')) AS collaborator_names
